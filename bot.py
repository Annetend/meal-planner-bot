...
def send_welcome(message):
    bot.reply_to(message, 
        ...
        "Я помогу вам составить сбалансированное меню с учётом калорий и бюджета.\n\n"
        "Выберите действие:",
...
"""
        ...
        for cat, recipe in menu.items():
            if recipe:
                ...
                result += f"{emojis[cat]} *{cat.capitalize()}*: не найден подходящий рецепт (попробуйте увеличить калории/бюджет)\n\n"
        
        result += f"📊 *Итого:*\n"
        result += f"🔥 {total_calories} ккал из {calories} ({int(total_calories / calories * 100)}%)\n"
        result += f"💰 {total_cost}₽ из {budget}₽ ({int(total_cost / budget * 100)}%)\n\n"
        
        if total_calories < calories * 0.8:
            result += "⚠️ *Рекомендация:* Калорийность ниже целевой. Рассмотрите добавление перекусов."
        elif total_cost > budget:
            result += "⚠️ *Рекомендация:* Бюджет превышен. Рассмотрите более экономичные рецепты."
        ...
        for day in days:
            ...
            for category in ['завтрак', 'обед', 'ужин']:
                if suitable_recipes[category]:
                    ...
                    result += f"  • {recipe['name']} ({recipe['calories']} ккал, {recipe['price']}₽)\n"
                    day_calories += recipe['calories']
                    day_cost += recipe['price']
            
            result += f"  Итого: {day_calories} ккал, {day_cost}₽\n\n"
            week_total_calories += day_calories
            week_total_cost += day_cost
        
        result += f"📊 *Неделя итого:*\n"
        result += f"🔥 {week_total_calories} ккал | 💰 {week_total_cost}₽\n"
        result += f"💰 Средний бюджет на день: {int(week_total_cost / 7)}₽"
...
• Похудение: {int(daily_calories * 0.8)} - {int(daily_calories * 0.9)} ккал
• Поддержание веса: {int(daily_calories)} ккал
• Набор массы: {int(daily_calories * 1.1)} - {int(daily_calories * 1.2)} ккал
        """
